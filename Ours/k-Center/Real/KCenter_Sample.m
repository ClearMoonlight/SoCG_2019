...
for k=2:5
    % Load inliers (n,d)
    load(['Data/Caltech/Original/k=',num2str(k),'/inlM.mat']);% inlM
    load(['Data/Caltech/Original/k=',num2str(k),'/kIf.mat']);%kInfo
    inlN=size(inlM,2);
    % Load inliers (n,d/8)
    load(['Data/Caltech/JL/k=',num2str(k),'/1-8/NinlM.mat']);%  NinlM
    for cno=1:candNo
        [k,cno]
        resTab=zeros(20,5);
        timTab=zeros(20,5);
        for ort=1:5
            ...
            oriP=[inlM,oulM];
            % Load outliers (n,d/8)
            load(['Data/Caltech/JL/k=',num2str(k),'/1-8/NoulM_',num2str(ort),'.mat']);
            tempP=[NinlM,NoulM];
            % Sampled data
        ...
        save(['Results/Caltech/Sample/k=',num2str(k),'/1-',num2str(title(cno)),'/resTab.mat'],'resTab');
        save(['Results/Caltech/Sample/k=',num2str(k),'/1-',num2str(title(cno)),'/timTab.mat'],'timTab');
    end
end